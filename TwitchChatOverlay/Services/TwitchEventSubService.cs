...
namespace TwitchChatOverlay.Services
{
    public class TwitchEventSubService
    {
        ...
        public async Task ConnectAsync(string accessToken, string clientId, string broadcasterUserId, string userId)
        {
            ...
            await this.ConnectWebSocketAsync(EventSubWssUrl);
        }
        ...
        private async Task ConnectWebSocketAsync(string url)
        {
            ...
            await this._webSocket.ConnectAsync(new Uri(url), this._cts.Token);
        ...
        private async Task ReceiveLoopAsync()
        {
            ...
            try
            {
                while (webSocket.State == WebSocketState.Open && !cancellationToken.IsCancellationRequested)
                {
                    ...
                    await this.HandleMessageAsync(messageBuilder.ToString());
                }
            }
            ...
            try
            {
                ...
                switch (messageType)
                {
                    case "session_welcome":
                        ...
                        if (sessionId != null)
                        {
                            await this._apiService.CreateEventSubSubscriptionsAsync(
                                this._accessToken, this._clientId, sessionId, this._broadcasterUserId, this._userId);
                        }
                    ...
                    case "session_reconnect":
                        ...
                        {
                            await this.ConnectWebSocketAsync(reconnectUrl);
                        }
        ...
        private void HandleNotification(JObject json)
        {
            ...
            var notification = subscriptionType switch
            {
            ...
            if (evt["message"]?["fragments"] is JArray fragArray)
            {
                foreach (var frag in fragArray)
                {
                    ...
                    if (fragType == "emote")
                    {
                        ...
                        var isAnimated = formats?.Any(f => f.ToString() == "animated") ?? false;
